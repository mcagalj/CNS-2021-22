...
route_credentials = RouteCredentials(scope=asymmetric.scope, password=asymmetric.password)
validate_access = validate_scope(scope=asymmetric.scope)

server_RSA_private = rsa.generate_private_key(
    public_exponent=65537,
    key_size=asymmetric.key_size
)
...
@router.post("/exchange/rsa-dh-params", dependencies=[Depends(validate_access)], response_model=RSAandDHParams)
def exchange_rsa_keys(client_key: PublicKey):
    global client_RSA_public

    try:
        client_RSA_public_serialized = client_key.key.encode()
        client_RSA_public = serialization.load_pem_public_key(client_RSA_public_serialized)
    except ValueError as error:
        raise HTTPException(status_code=400, detail=repr(error))

    return RSAandDHParams(
        key=server_RSA_public_serialized,
        dh_params=dh_parameters_serialized
    )


@router.post("/exchange/dh", dependencies=[Depends(validate_access)], response_model=SignedPublicKey)
def exchange_signed_dh_keys(client_key: SignedPublicKey):
    ...
    try:
        ...
        client_DH_public = serialization.load_pem_public_key(client_DH_public_serialized)
        ...
        signature = server_RSA_private.sign(
            data=dh_parameters_serialized + server_DH_public_serialized + client_DH_public_serialized,
            padding=padding.PSS(
                mgf=padding.MGF1(hashes.SHA256()),
                salt_length=padding.PSS.MAX_LENGTH
            ),
...
def read_challenge():
    ...
    if not server_DH_private:
        raise HTTPException(status_code=400, detail="Something went wrong. Re-run the protocol from start.")
